<div class="min-h-screen mt-10 p-6 bg-slate-50 dark:bg-slate-900">
    <div class="">
        <!-- Header -->
        <div class="my-5">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Package Register</h1>
            <p class="text-slate-500 dark:text-slate-400">Choose how many person will join ? </p>
        </div>

        <!-- Booking Form Card -->
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-6">
            <form class="space-y-6" wire:submit.prevent="registerPackage">
                <!-- Total Amount -->
                <div>
                ...
                <div>
                    ...
                    <div class="relative">
                        {{-- <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400">$</span> --}}
                        <input type="number" id="no_of_person" name="no_of_person" required min="0"

                            wire:model.live="no_of_person" 
                            class="w-full pl-8 pr-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg
                                   focus:ring-2 focus:ring-teal-500 dark:focus:ring-teal-400 focus:border-transparent
                                   bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400"
                            placeholder="Enter Number Of Persion">
                    </div>
                    @error('no_of_person')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
